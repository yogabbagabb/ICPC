...
class Robot
{
	...
	public String act(String code, int repeat)
	{
		...
		else
		{
			...
			if (closestRobotInfo[0] != CrashingRobots.xBoundary + CrashingRobots.yBoundary && repeat >= closestRobotInfo[0])
			{
	...
	{			
		int minDistance = CrashingRobots.xBoundary + CrashingRobots.yBoundary;
	...
	public int getDistanceOfClosestWall()
	{
		switch (dir)
		{
			case NORTH:
			{
				return CrashingRobots.yBoundary - this.y + 1;	
			}
			...
			case EAST:
			{
				return CrashingRobots.xBoundary - this.x + 1;
			}
