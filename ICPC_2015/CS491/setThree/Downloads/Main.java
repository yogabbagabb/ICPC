...
public class Main {
	...
	public static void main (String[]args)
	{
		...
		while (scenarios > 0)
		{
			...
			Main.updateInvalidPositions(invalidPositions, queenPosition);
			...
			if (yQueenPos == 0)
			{
				finalList = Main.goDeepHelper(true, invalidPositions, developingPositions);
			}
			else
			{
				finalList = Main.goDeepHelper(false, invalidPositions, developingPositions);
			}
	...
	public static java.util.ArrayList<int[]>goDeepHelper(boolean queenIsFirst, java.util.HashSet<Integer> invalidPositions, java.util.List<Integer> developingPositions)
	{
		...
		if (queenIsFirst)
		{
			for (int i = 8; i < 16; i++)
			{
				...
				Main.goDeep(7, i, invalidPositions, developingPositions, finalList);
			}
		}
		else
		{
	...
	public static void goDeep(int remainingQueens, int position, java.util.HashSet<Integer> invalidPositions, java.util.List<Integer> developingPositions, java.util.List<int[]>finalList)
	{
		...
		else
		{
			...
			Main.updateInvalidPositions(newInvalidPositions, position);
			
			
			
			boolean theQueenIsNext = (position/8 + 1 == queenPosition/8);
			int[] nextPositions = Main.findNextPosition(position, newInvalidPositions, theQueenIsNext);
			

			for (int i = 0; i < nextPositions.length && nextPositions[i] != 0; i++)
				Main.goDeep(remainingQueens - 1, nextPositions[i], newInvalidPositions, newDevelopingPositions, finalList);			
